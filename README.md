...
| 停止当前任务（连按两次） | `Esc` `Esc` | `Esc` `Esc` |

`Esc` 是「逐层退」：先关掉最上面那层 UI——命令面板 / 输入历史 / 右键菜单 / 行内重命名 / 设置·关于弹窗；这一层都没有、且任务正在运行时，`Esc` 进入「待确认停止」态（提示行显示「再按一次 Esc 停止任务」），2 秒内再按一次才真正停止（等价于输入区的「停止」按钮）。之所以不做「一按就停」：`Esc` 是高频手滑键，而停止任务会丢掉正在进行的回合。

输入框内：`Enter` 发送、`Shift+Enter` 换行、`↑` / `↓` 翻阅输入历史。

不用刻意背：Composer 底部的提示行会**按当前状态**显示最相关的一条——对话区被上翻时提示「回到最新」、任务运行中时提示「连按两次 `Esc` 停止任务 + 排队说明」、按下一次 `Esc` 后提示「再按一次 `Esc` 停止任务」、多标签页时提示标签页键（都不命中时回落为安全策略说明）。
