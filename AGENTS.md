...
-   **定价 / 计费**：价目表与抓取解析都在插件 `litework/builtin_plugins/pricing-plugin/`（改价只改它的 `pricing.json`，勿在主程序硬编码价格）；主程序仅有基类与通用分时窗口求值 `litework/llm/pricing_provider.py`、以及 `app.resolve_pricing` 的分层回退；
...
是 office-plugin / ocr-plugin / webfetch-plugin / collab-\* / pricing-plugin 等插件的**上游事实源**。

1.  **许可红线**：lite-work-plugins 整体为 **MIT**。凡来自（或同步自）该仓库的  
    插件/技能代码（含 pricing-plugin），**严禁添加 Apache 头**（`# SPDX-License-Identifier: Apache-2.0`  
...
4.  **「同步内置」＝等价于用户在设置里点升级**：把社区包代码**原样**引入内置  
    ...
    额外改动（Agent 工具白名单、提示词、前端行为等——用户手动  
    升级社区插件时这些都不会变，同步也不应变）；
5.  **已知边界**：内置 `litework/tools/office.py` 已按本原则与社区 v1.4.1 同源  
    （MIT、无许可证头）；`litework/tools/ocr.py` 已按本原则与社区 ocr-plugin  
    v1.1.0 同源（MIT、无许可证头，mypy 豁免同 office）；`web.py` 仍是主仓库  
    原生文件，待社区对齐时再按同一原则处理，不要顺手「统一」它。